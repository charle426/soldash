    ...
    useEffect(() => {
        async function fetchedTokens() {
            ...
            await axios.request(requestOptions)
                ...
                .catch(err => {setErrMsg(err.message), setErrToggle(true)});
    ...
    return (
        <div className="w-full">
            ...
            <Table className="relative font-medium *:text-[0.8em]">
                {/* {error message so the client knows what went wrong} */}
                {errMsg && errToggle && <div className="text-white bg-red-500 flex items-center justify-center gap-2 rounded-lg p-3 absolute top-16 left-[50%] z-10 "><p className="font-medium">{errMsg}</p>     <span className=" rounded-full cursor-pointer w-4 h-4 flex justify-center items-center bg-black text-[0.6rem]" onClick={() => setErrToggle(false)}>X</span></div>}
                    ...
                    trendingToken.length === 0 ? (
                        ...
                        trendingToken.map((tokens: any, index: number) => {
                            ...
                            return (
                                ...
                                <TableBody className={tokenInfo.address === tokens.address ? "rounded-2xl m-2 cursor-pointer divide-y odd:bg-blue-200 even:bg-blue-200" : "divide-y odd:bg-slate-100 even:bg-white"}
                                    onClick={() => {
                                        setTokenInfo({
                                ...
                                >
                                    <TableRow>
                                            ...
                                            <TooltipProvider>
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <span className="underline cursor-pointer"><Link className="flex gap-2" href={`/tokens?address=${tokenInfo.address}`}>{tokens.name}</Link></span>
                                                    </TooltipTrigger>
                                        ...
                                        <TableCell className="">
                                            {tokenInfo.address === tokens.address ?  <Link href={`/tokens?address=${tokens.address}`} className="text-blue-500 underline
                                        ...
                                        <TableCell className="text-right">{tokens.market_cap !== null ? formatPrice(tokens.market_cap) : "N/A"}</TableCell>
                                    </TableRow>
                                </TableBody>
