        ...
        const metaData = await Promise.all(
          data.map(async (token: Tokens) => {
            ...
            try {
              ...
              console.error(
                "Metadata fetch failed for token",
                token.address,
                err
              );
  ...
  return (
    <div className="w-full">
      <div className="gap-2 flex justify-end">
        <Button
          variant={tokenInfo.urlName == "tokens" ? "default" : "outline"}
          className="mb-2"
          onClick={() =>
            setTokenInfo((prev) => ({
              ...prev,
        ...
        <Button
          variant={tokenInfo.urlName == "trending" ? "default" : "outline"}
          onClick={() =>
            setTokenInfo((prev) => ({
              ...prev,
              urlName: "trending",
              url: "https://pro-api.solscan.io/v2.0/token/trending?limit=20",
            }))
          }
        >
        ...
        {trendingToken.length === 0
              ...
              return (
                <TableBody key={skeleton} className="m-2">
                  <TableRow className="mb-5">
                    ...
                    <TableCell className=" fle justify-end items-center">
                      <Skeleton className="h-4 w-[120px] rounded-xl" />
                    </TableCell>
                    <TableCell className="t fex justify-end items-center">
          ...
          : trendingToken.map((tokens: Tokens, index: number) => {
              return (
                <TableBody
                  className={
                    tokenInfo.address === tokens.address
                      ? "w-full rounded-2xl m-2 cursor-pointer divide-y odd:bg-blue-200 even:bg-blue-200"
                      : "w-full m-2 divide-y odd:bg-slate-100 even:bg-white"
                  }
                  onClick={() => {
                    setTokenInfo({
                ...
                >
                  <TableRow>
                    ...
                    <TableCell className="flex justify-start items-center align-middle  gap-1 md:gap-2">
                      ...
                      <span
                        className={
                          tokens.price_change_24h > 0
                            ? "text-green-500 text-right flex gap-[2px] items-center justify-center"
                            : tokens.price_change_24h < 0
                            ? "text-red-500 text-right flex gap-1 items-center justify-center"
                      ...
                      >
                        {tokens.price_change_24h > 0 ? (
                          <ArrowUp
                            size={15}
                            className="text-green-500 text-right text-[0.5rem]"
                          />
                        ) : tokens.price_change_24h < 0 ? (
                          <ArrowDown
                            size={15}
                            className="text-red-500 text-right text-[0.5em]"
                          />
                        ) : (
                          "--"
                        )}
                        {tokens.price_change_24h !== null
                          ? formatPrice(tokens.price_change_24h)
                          : "0"}
                        %
                      </span>
                    </TableCell>
                    <TableCell className="">
                      {tokens.market_cap ? (
                        <span>{formatUSD(tokens.market_cap)}</span>
                      ) : (
                        <span>N/A</span>
                      )}
                    </TableCell>
